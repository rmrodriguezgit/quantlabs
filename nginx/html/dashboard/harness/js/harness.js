...
let attachedFiles=[], currentSessionId='default', currentProjectId='quantlabs-workspace', selectedFileId=null, activeFileIds=new Set(), conversations=[], projects=[], currentProject=null, microserviceCatalog={nodes:[],edges:[],recommended_flows:[]}, lastAgents=[], lastTools=[], lastSystem={}, lastMemory={}, lastStatus={};
...
async function loadProjects(preferred){
  ...
  currentProjectId=preferred&&projects.some(p=>p.id===preferred)?preferred:(currentProjectId&&projects.some(p=>p.id===currentProjectId)?currentProjectId:projects[0].id);
  const detail=await get(`/projects/${encodeURIComponent(currentProjectId)}`);
  currentProject=detail.project||projects.find(p=>p.id===currentProjectId)||projects[0];
}
async function loadMicroserviceCatalog(){
  try{microserviceCatalog=await get('/microservices/catalog')}catch(e){microserviceCatalog={nodes:[],edges:[],recommended_flows:[]}}
}
...
function renderMicroserviceCatalog(){
  const nodes=microserviceCatalog.nodes||[];
  const edges=microserviceCatalog.edges||[];
  const flows=microserviceCatalog.recommended_flows||[];
  const groups=nodes.reduce((acc,node)=>{const key=node.category||'Tool';(acc[key]=acc[key]||[]).push(node);return acc},{});
  const nodeCards=Object.entries(groups).map(([category,items])=>`<div class="micro-group"><h4>${esc(category)}</h4>${items.map(node=>`<article class="micro-node"><div><strong>${esc(node.label||node.id)}</strong><p>${esc(node.purpose||'Microservicio disponible')}</p></div><div class="micro-tags">${(node.inputs||[]).slice(0,3).map(x=>`<span>in: ${esc(x)}</span>`).join('')}${(node.outputs||[]).slice(0,3).map(x=>`<span>out: ${esc(x)}</span>`).join('')}</div><small>Reutilizable: ${esc((node.reusable_for||[]).join(', ')||'general')}</small></article>`).join('')}</div>`).join('');
  const edgeRows=edges.map(edge=>`<li><span>${esc(edge.from)} → ${esc(edge.to)}</span><span class="panel-mini">${esc(edge.label)}</span></li>`).join('')||'<li>Sin conexiones sugeridas para este rol.</li>';
  const flowRows=flows.map(flow=>`<li><span>${esc(flow.name)}</span><span class="panel-mini">${esc((flow.nodes||[]).join(' → '))}</span><small>${esc(flow.use||'')}</small></li>`).join('')||'<li>Sin flujos recomendados.</li>';
  return `<div class="micro-catalog"><div class="micro-hero"><div><small>Catálogo reutilizable</small><strong>${nodes.length.toLocaleString()} microservicios</strong></div><span>tipo n8n</span></div><div class="micro-map">${nodeCards}</div><div class="panel-card"><h4>Conexiones sugeridas</h4><ul class="panel-list micro-edge-list">${edgeRows}</ul></div><div class="panel-card"><h4>Flujos reciclables</h4><ul class="panel-list micro-flow-list">${flowRows}</ul></div></div>`;
}
async function openPlugins(){setActiveNav(el.pluginsNav);openPanel('Complementos','Catálogo de microservicios','<div class="panel-card"><h4>Cargando</h4><p>Consultando nodos, entradas, salidas y flujos reutilizables.</p></div>');await loadMicroserviceCatalog();el.sidePanelBody.innerHTML=renderMicroserviceCatalog()}
...
async function openProject(){setActiveNav(el.projectNav);openPanel('Proyecto','Proyecto de conocimiento','<div class="panel-card"><h4>Cargando</h4><p>Consultando memoria persistente, RAG y archivos del proyecto.</p></div>');await loadProjects(currentProjectId);const p=currentProject||{};const rag=p.rag_stats||{};const byAgent=Object.entries(rag.by_agent||{}).map(([agent,count])=>`<span>${esc(agent)} · ${Number(count).toLocaleString()}</span>`).join('')||'<span>Sin RAG todavía</span>';const projectOptions=projects.map(item=>`<option value="${esc(item.id)}" ${item.id===currentProjectId?'selected':''}>${esc(item.name||item.id)}</option>`).join('');const systemPayload={compute:{gpu:'Tesla T4',mode:'cuda'},...lastSystem,compute:{gpu:'Tesla T4',mode:'cuda',...(lastSystem.compute||{})}};const detectedArtifacts=collectArtifacts();const artifacts=renderArtifactRows(detectedArtifacts);const files=attachedFiles.map(f=>`<li class="panel-action"><span>${esc(f.name)}</span><span class="panel-mini">${esc(f.ext||'archivo')} · ${p.files?.includes(f.id)?'vinculado':'disponible'}</span></li>`).join('')||'<li>Sin archivos cargados.</li>';const tasks=(lastMemory.tasks||[]).map(t=>`<li class="panel-action"><span>${esc(t.agent||'agente')}: ${esc(t.status||'sin estado')}</span><span class="panel-mini">${taskDuration(t)}</span></li>`).join('')||'<li>Sin tareas activas.</li>';el.sidePanelBody.innerHTML=`<div class="project-grid"><div class="panel-card project-card-main"><h4>▣ Proyecto persistente</h4><label class="project-select-label">Proyecto<select id="projectSelector">${projectOptions}</select></label><p>${esc(p.description||'Memoria persistente del Harness.')}</p><div class="project-actions"><button id="newProjectBtn" type="button">Nuevo proyecto</button><button id="linkFilesBtn" type="button">Vincular archivos activos</button></div></div><div class="panel-card"><h4>RAG por especialista</h4><div class="artifact-badges">${byAgent}</div><p class="panel-mini">${Number(rag.items||0).toLocaleString()} memorias del proyecto. Borrar chats no elimina este RAG.</p></div><div class="panel-card"><h4>PROJECT.md</h4><pre class="project-md">${esc(p.project_md_preview||'Sin PROJECT.md')}</pre></div><div class="panel-card"><h4>Archivos del proyecto</h4><ul class="panel-list">${files}</ul></div><div class="panel-card"><h4>Contexto de sesión</h4>${renderContextPanel(lastMemory.metadata||{})}</div><div class="panel-card"><h4>Sistema</h4><pre class="system-json">${esc(JSON.stringify(systemPayload,null,2))}</pre></div><div class="panel-card"><h4>Artefactos de chat</h4><div class="panel-mini" style="margin-bottom:10px">${Number(detectedArtifacts.length).toLocaleString()} detectado${detectedArtifacts.length===1?'':'s'} en memoria y mensajes</div><ul class="panel-list artifact-list">${artifacts}</ul></div><div class="panel-card"><h4>Tareas · tiempo de respuesta</h4><ul class="panel-list">${tasks}</ul></div><div class="panel-card"><h4>Accesos</h4><div class="quick-row"><a href="/dashboard/">Dashboard</a><a href="/jupyter/" target="_blank">Jupyter</a><a href="/llm/">LLM Local</a><a href="/dashboard/api/">API</a></div></div></div>`}
el.closeSidePanel.onclick=closePanel;el.panelBackdrop.onclick=closePanel;el.searchNav.onclick=openSearch;el.pluginsNav.onclick=openPlugins;el.automationsNav.onclick=openAutomations;el.artifactsNav.onclick=openArtifacts;el.projectNav.onclick=openProject;el.chatSearch.oninput=renderConversationList;el.sidePanelBody.onclick=async e=>{const copy=e.target.closest('[data-copy-artifact]');if(copy){copyText(copy.dataset.copyArtifact||'');copy.textContent='✓';setTimeout(()=>copy.textContent='⧉',900);return}const hit=e.target.closest('[data-open-chat]');if(hit){loadSession(hit.dataset.openChat,{preserveScroll:false});closePanel();return}if(e.target.closest('#newProjectBtn')){const name=window.prompt('Nombre del nuevo proyecto','Nuevo proyecto Harness');if(!name)return;const created=await api('/projects',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name})});currentProjectId=created.project.id;await openProject();return}if(e.target.closest('#linkFilesBtn')){await api(`/projects/${encodeURIComponent(currentProjectId)}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({files:[...activeFileIds]})});await openProject();return}};
el.sidePanelBody.onchange=async e=>{if(e.target?.id==='projectSelector'){currentProjectId=e.target.value;await openProject()}};
...
async function sendPrompt(message){
  ...
  try{
    const data=await api('/chat',{method:'POST',headers:{'Content-Type':'application/json'},signal:currentChatController.signal,body:JSON.stringify({session_id:currentSessionId,project_id:currentProjectId,message:outboundMessage,agent:selectedAgent,file_ids:[...activeFileIds],show_rag:true})});
