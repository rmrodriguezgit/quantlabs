...
def portfolio_markowitz(tickers: str=','.join(DEFAULT_TICKERS), benchmark: str=DEFAULT_BENCHMARK, period: str='2y'):
    ...
    rng=np.random.default_rng(42); sw=rng.dirichlet(np.ones(len(names)),size=3000); sr=sw@mu; sv=np.sqrt(np.einsum('ij,jk,ik->i',sw,cov,sw)); ssh=np.divide(sr,sv,out=np.full_like(sr,np.nan),where=sv>0)
